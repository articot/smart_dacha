...
enum HeaterMode { AUTO, FORCE_ON, FORCE_OFF };
...
enum GsmRxState { GSM_IDLE, GSM_WAIT_BODY };
...
String buildStatusSMS() {
    ...
    msg += " (";
    msg += (heaterMode == AUTO)     ? "Auto"     :
           (heaterMode == FORCE_ON) ? "Force ON" : "Force OFF";
    msg += ")";
    return msg;
}
...
void processSMS(const String& sender, String text) {
    ...
    if (sender != String(ADMIN_PHONE)) return;
...
void applyHeaterLogic() {
    if (heaterMode == FORCE_ON)  { setRelay(true);  return; }
    if (heaterMode == FORCE_OFF) { setRelay(false); return; }
    ...
    for (int i = 0; i < NUM_ROOMS; i++) {
        if (!rooms[i].online) continue;
        if (rooms[i].temp <  TEMP_THRESHOLD)             anyBelowThreshold  = true;
        if (rooms[i].temp <= TEMP_THRESHOLD + HYSTERESIS) allAboveHysteresis = false;
    }

    if (!relayState && anyBelowThreshold)  setRelay(true);
    if ( relayState && allAboveHysteresis) setRelay(false);
...
<body>
<h1>Smart Dacha</h1>
<div class='subtitle'>auto-refreshes every 10 s</div>
<div class='grid'>)");

    for (int i = 0; i < NUM_ROOMS; i++) {
        html += "<div class='card";
        if (!rooms[i].online) html += " offline";
        html += "'><div class='room'>" + String(rooms[i].name) + "</div>";
        if (rooms[i].online) {
            html += "<div class='temp'>"  + String(rooms[i].temp,     1) + "&deg;</div>";
            html += "<div class='hum'>"   + String(rooms[i].humidity, 1) + "% RH</div>";
        } else {
            html += "<div class='temp'>--</div><div class='hum'>offline</div>";
        }
        html += "</div>";
    }
    ...
    html += "<div class='state'>State: <span class='" + String(relayState ? "on'>ON" : "off'>OFF") + "</span></div>";

    // Mode radio buttons
    html += "<div class='mode-switch'>";
    html += "<label class='radio-lbl'><input type='radio' name='hmode' value='auto' onchange=\"switchMode(this.value)\"";
    if (isAuto) html += " checked";
    html += "> Auto</label>";
    html += "<label class='radio-lbl'><input type='radio' name='hmode' value='manual' onchange=\"switchMode(this.value)\"";
    if (!isAuto) html += " checked";
    html += "> Manual</label>";
    html += "</div>";
...
void handleApiStatus() {
    ...
    doc["mode"]   = (heaterMode == FORCE_ON) ? "force_on" :
                    (heaterMode == FORCE_OFF) ? "force_off" : "auto";
